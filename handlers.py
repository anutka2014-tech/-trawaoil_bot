...
async def show_card(callback: CallbackQuery, product: dict, keyboard) -> None:
    ...
    Если фото есть — редактирует медиа или отправляет новое фото.
    ...
    if photo_url:
        try:
            ...
            return  # фото загрузилось — выходим
        except Exception as e:
            logger.warning(f"Фото не загрузилось ({photo_url[:60]}…): {e}. Показываю текст.")
